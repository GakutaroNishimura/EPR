...
for j in range(len(df_bf)):
    ...
    for i in range(Npeak):
        ...
        gr = ROOT.TGraphErrors(len(df_bf[j].time), np.array(df_bf[j].time), np.array(df_bf[j].signal), np.array([0.0 for i in range(len(df_bf[j].time))]), np.array([Noise for i in range(len(df_bf[j].time))]))
        ...
        c1.SaveAs(path + "error/e_bf" + str(j) + "_peak" +  str(i) + ".pdf")
        c1.SaveAs(path + "error/e_bf" + str(j) + "_peak" +  str(i) + ".png")
        #time.sleep(10000)
...
for j in range(len(df_af)):
    ...
    for i in range(Npeak):
        ...
        Fit_center = 0.000533512052921227-Npeak/2/F_mod+i/F_mod
        # create a TGraph with the data
        gr = ROOT.TGraphErrors(len(df_af[j].time), np.array(df_af[j].time), np.array(df_af[j].signal), np.array([0.0 for i in range(len(df_af[j].time))]), np.array([Noise for i in range(len(df_af[j].time))]))
        # set the fit function and the fit range
        f = ROOT.TF1("f", "[0]-gaus(x, [1], [2], [3])", Fit_center-Fit_Range, Fit_center+Fit_Range)
        # set initial parameter values for the fit
        f.SetParameters(0, 0.003, Fit_center, 6*10**(-5))
        ...
        ROOT.gStyle.SetOptFit(1)
        c1 = ROOT.gROOT.FindObject("c1")
        c1.SaveAs(path + "error/e_af" + str(j) + "_peak" +  str(i) + ".pdf")
        c1.SaveAs(path + "error/e_af" + str(j) + "_peak" +  str(i) + ".png")
        #time.sleep(10000)
...
gr.Draw("AP")
c1 = ROOT.gROOT.FindObject("c1")
c1.SaveAs(path + "EPRfreq.pdf")
c1.SaveAs(path + "EPRfreq.png")
